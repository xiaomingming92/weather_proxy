...
export function registerAuditTools(server: McpServer) {
    ...
    async (
      ...
      try {
        ...
        if (sinceMinutes !== undefined)
          where.createdAt = {
            gte: new Date(Date.now() - sinceMinutes * 60 * 1000),
          };
        ...
        if (logs.length === 0) {
          const f: string[] = [];
          if (targetType) f.push(`targetType=${targetType}`);
          if (action) f.push(`action=${action}`);
          if (targetId) f.push(`targetId=${targetId}`);
          if (planKeyword) f.push(`planKeyword=${planKeyword}`);
          if (keyword) f.push(`keyword="${keyword}"`);
          if (sinceMinutes !== undefined)
            f.push(`sinceMinutes=${sinceMinutes}`);
          return textResponse(
            `=== 开发操作审计日志 ===\n\n未找到匹配的审计记录（条件: ${f.length > 0 ? f.join(', ') : '无条件'}）。\n\n可能原因: 数据库未运行、尚无相关开发操作记录、或 filter 过于严格。`
          );
        }
        const fl: string[] = [];
        if (targetType) fl.push(`targetType=${targetType}`);
        if (action) fl.push(`action=${action}`);
        if (targetId) fl.push(`targetId=${targetId}`);
        if (planKeyword) fl.push(`planKeyword=${planKeyword}`);
        if (keyword) fl.push(`keyword="${keyword}"`);
        if (sinceMinutes !== undefined) fl.push(`最近${sinceMinutes}分钟`);
        const lines = [
          `=== 开发操作审计日志 (条件: ${fl.length > 0 ? fl.join(', ') : '无过滤条件（最近全部）'}) ===`,
          `共 ${logs.length} 条记录`,
        ];
        for (let i = 0; i < logs.length; i++) {
          const l = logs[i];
          lines.push(
            `[${i + 1}] ${l.createdAt.toISOString()}`,
            `    action: ${l.action} | targetType: ${l.targetType} | targetId: ${l.targetId || '(无)'}`
          );
          if (l.reason) lines.push(`    reason: ${l.reason}`);
          if (l.planKeyword) lines.push(`    planKeyword: ${l.planKeyword}`);
        }
        if (planKeyword) {
          lines.push(
            '=== Plan 分组 ===',
            `planKeyword=${planKeyword} 的操作链:`
          );
          for (const l of logs)
            lines.push(
              `  ${l.createdAt.toISOString().slice(11, 19)} ${l.action}`
            );
        }
        lines.push(
          '',
  ...
  server.registerTool(
    ...
    {
      description:
        '记录一次开发操作到 DevOperation 表（ADD-7）。AI 助手在对代码进行任何修改/创建/删除操作后，必须调用此工具记录操作审计。\n\n**targetId 路径格式（强制）**：必须使用相对于 workspace 根目录的路径，禁止绝对路径。\n- ✅ src/middleware.ts\n- ✅ ${MAGIC_DIR}/plans/xxx.md\n- ❌ /absolute/path/to/src/middleware.ts',
      inputSchema: z.object({
        action: z
          .string()
          .describe(
            "操作类型: 'MODIFY', 'CREATE', 'DELETE', 'DOC_UPDATED', 'DOC_CREATED' 等"
          ),
        targetType: z
          .string()
          .describe(
            "目标类型: 'API_ROUTE', 'COMPONENT', 'SCHEMA', 'RULE', 'DOC', 'PLAN', 'SPEC', 'SKILL', 'AGENT' 等"
          ),
        ...
        beforeState: z
          .string()
          .describe(
            '操作前的状态（JSON 字符串）。必填，记录变更前的文件内容或配置状态，用于审计回溯。'
          ),
        afterState: z
          .string()
          .describe(
            '操作后的状态（JSON 字符串）。必填，记录变更后的文件内容或配置状态，用于审计回溯。'
          ),
    ...
    async (
      ...
      try {
        ...
        if (
          targetId &&
          (targetId.startsWith('/') || /^[A-Z]:\\/.test(targetId))
        )
          pathWarnings.push(
            `⚠️ targetId 使用了绝对路径: "${targetId}"。请使用相对路径。`
          );
        ...
        let systemUser = await (
          prisma.addUser as Record<string, (...a: unknown[]) => unknown>
        ).findUnique({
        ...
        if (pathWarnings.length > 0) {
          lines.push('');
          lines.push(...pathWarnings);
        }
        lines.push(
          '',
          `📋 落库回查（必须执行）:`,
          targetId
            ? `  query_audit_logs({ targetId: "${targetId}" }) — 确认本条记录已写入`
            : ''
        );
        return textResponse(lines.join('\n'));
      } catch (error) {
        return errorResponse(
          `记录开发操作失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
