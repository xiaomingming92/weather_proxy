...
> ⚠️ **偏差声明**：
> - 审计 API 使用 `agentAudit("PHASE", message, extra)`（event-based），需实现三通道输出（console + file + AuditLog 表）。add-coder 配套示例项目提供了审计日志器参考实现，见 `docs/weather-proxy/knowledge/02-规范/《ADD可审计开发范式案例参考》.md`。
...
```
weather-proxy-多轮对话能力专家链路优化统一状态管理-plan-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-7轮原子事务拆分-add-route-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-7轮原子事务交接-handoff-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-方案评审-review-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-round1-类型收敛-thinkingLevel路由-spec-review-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-round2-响应策略裁决-spec-review-v1.md
weather-proxy-多轮对话能力专家链路优化统一状态管理-round3-专家注册分析上下文-spec-review-v1.md
```

### 命名带来的好处

1. **可追溯**：从文件名直接回答"什么需求、第几轮、什么工作、第几版本"
2. **可聚合**：`ls weather-proxy-*` 一条命令捞出全部关联文件
...
| 目录 | 对应的工作 |
|------|-----------|
| `weather-proxy-type-convergence-v1` | 第1轮：类型收敛 + thinkingLevel 路由 |
| `weather-proxy-response-strategy-v1` | 第2轮：响应策略裁决 |
| `weather-proxy-expert-registry-v1` | 第3轮：专家注册 + 分析上下文 |
| `weather-proxy-pipeline-integration-v1` | 第4轮：管线集成 |
| `weather-proxy-semantic-cache-v1` | 第5轮：语义缓存 |
| `weather-proxy-evolution-loop-v1` | 第6轮：演化闭环 |
| `weather-proxy-audit-pipeline-v1` | 第7轮：审计管线 |
...
## 六、实际案例：weather-proxy 7 轮原子事务

完整流程见：

| 阶段 | 文件 | 说明 |
|------|------|------|
| 需求方案 | `.qoder/plans/weather-proxy-多轮对话能力专家链路优化统一状态管理-plan-v1.md` | 总体设计 |
| 拆分拓扑 | `.qoder/plans/weather-proxy-多轮对话能力专家链路优化统一状态管理-7轮原子事务拆分-add-route-v1.md` | 7 轮依赖拓扑 |
| 方案评审 | `.qoder/reviews/weather-proxy-多轮对话能力专家链路优化统一状态管理-方案评审-review-v1.md` | 可行性验证 |
| 交接手册 | `.qoder/plans/weather-proxy-多轮对话能力专家链路优化统一状态管理-7轮原子事务交接-handoff-v1.md` | 轮间输入输出 |
| 第1轮 spec | `.qoder/specs/weather-proxy-type-convergence-v1/` | 类型收敛三件套 |
| 第1轮评审 | `.qoder/reviews/weather-proxy-多轮对话能力专家链路优化统一状态管理-round1-类型收敛-thinkingLevel路由-spec-review-v1.md` | spec 人工 review |
| ... | ... | 第2-7 轮同理 |
...
| **禁止项** | 架构级**设计约束**："禁止删除全局 weather-proxy collection" | 实现级**操作禁止**："禁止一次性迁移超过 3 个 Expert" |
