...
export function registerContextTools(server: McpServer) {
    ...
    'get_project_context',
    {
      ...
      inputSchema: z.object({
        scope: z
          ...
          .optional()
          .describe(
            "获取信息的范围: 'structure' 仅目录结构, 'rules' 仅项目规则, 'package' 仅包信息, 'add-state' ADD 工作流状态, 'all' 全部"
          ),
      }),
    },
      ...
      try {
        ...
        if (scope === 'all' || scope === 'package') {
          ...
          if (pkg) {
            ...
            parts.push(
              `技术栈: Next.js ${parsed.dependencies?.next || 'unknown'} + TypeScript + Prisma + LangGraph`
            );
            parts.push('');
            parts.push('核心依赖:');
            const keyDeps = [
            ...
            for (const dep of keyDeps) {
              const ver =
                parsed.dependencies?.[dep] || parsed.devDependencies?.[dep];
        ...
        if (scope === 'all' || scope === 'rules') {
          const rules = await readFileSafe(
            join(PROJECT_ROOT, MAGIC_DIR, 'rules', 'project_rules.md')
          );
          if (rules) {
        ...
        if (scope === 'all' || scope === 'structure' || scope === 'add-state') {
          ...
          if (existsSync(plansDir)) {
            ...
            if (planFiles.length > 0) {
              ...
              const handoffFiles = allPlanFiles.filter(f =>
                f.includes('handoff')
              );
              const hasHandoff = handoffFiles.some(f =>
                f.toLowerCase().includes(planKeyword.toLowerCase())
              );
              parts.push(
                ` add-route: ${addRouteFile ? '✅ ' + addRouteFile : '❌ 缺失（需回退 Step 0.5）'}`
              );
              parts.push(
                ` handoff: ${hasHandoff ? '✅ 已生成' : '❌ 缺失（需回退 Step 0.5）'}`
              );
            } else {
          ...
          if (existsSync(reviewsDir) && activePlan) {
            ...
            if (matchingReview) {
              ...
              const reviewContent =
                (await readFileSafe(join(reviewsDir, matchingReview))) || '';
              ...
              if (reviewP0Count + reviewP1Count > 0 && planContent) {
                ...
                for (const ind of indicators) {
                  if (planContent.toLowerCase().includes(ind.toLowerCase()))
                    hit++;
                }
                reviewBackflowRate = Math.min(
                  100,
                  Math.round(
                    (hit / Math.max(reviewP0Count + reviewP1Count, 1)) * 100
                  )
                );
              }
              parts.push('');
              ...
              parts.push(
                `  P0 问题: ${reviewP0Count} 个, P1 问题: ${reviewP1Count} 个`
              );
              parts.push(`  回流状态: 约 ${reviewBackflowRate}%`);
              parts.push(
                reviewBackflowRate < 70
                  ? '  ⚠️ Review 结论未充分回流至 Plan — 需执行 0.6.5 卡位'
                  : '  ✅ Review 结论基本回流至 Plan'
              );
            } else {
              parts.push('');
          ...
          if (activePlan) {
            ...
            const specDirs = existsSync(specsDir)
              ...
              : [];
            const matchingSpec = specDirs.find(d =>
              d.toLowerCase().includes(planKeyword.toLowerCase())
            );
            parts.push('');
            if (matchingSpec) {
              const hasSpec = existsSync(
                join(specsDir, matchingSpec, 'spec.md')
              );
              const hasTasks = existsSync(
                join(specsDir, matchingSpec, 'tasks.md')
              );
              const hasChecklist = existsSync(
                join(specsDir, matchingSpec, 'checklist.md')
              );
          ...
          if (!activePlan) {
            ...
            if (!reviewFileName) {
              ...
              reviewP0Count + reviewP1Count > 0 &&
              reviewBackflowRate < 70
            ) {
              todoItems.push(
                '1. [0.6.5] Review 结论回流至 Plan — P0/P1 未写入 Plan 体'
              );
              todoItems.push(
                '2. [check_dps] DPS 门禁预计不通过（回流完整度 < 70%）'
              );
            }
            ...
            if (
              !sd.some(d => d.toLowerCase().includes(planKw.toLowerCase())) &&
              reviewBackflowRate >= 70
            ) {
              todoItems.push('3. [Step 0] 生成 Specs 三元组');
            }
            if (todoItems.length === 0) {
              todoItems.push('✅ ADD 就绪 — check_dps ≥ 80 后可进入 Step 1');
            }
          }
          for (const item of todoItems) parts.push(item);
          parts.push('');
          parts.push(
            '快速指令: 说「执行 add-paradigm Step 0」进入文档先行流程'
          );
          parts.push(
            '          说「将 Review 结论回流至 Plan」触发 0.6.5 卡位'
          );
        }
        ...
        if (scope === 'all' || scope === 'structure') {
          ...
          const magicDirs = [
            ...
            {
              dir: 'skills',
              desc: 'SKILL 行为定义（add-paradigm / session-init）',
            },
          ...
          for (const { dir, desc } of magicDirs) {
            ...
            if (existsSync(fullPath)) {
              const entries = await readdir(fullPath);
              parts.push(
                `  ${MAGIC_DIR}/${dir}/ (${entries.length} 项) — ${desc}`
              );
            }
          }
        }
        ...
        return errorResponse(
          `获取项目上下文失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
  ...
  server.registerTool(
    ...
    {
      description:
        '获取 Prisma 数据库 Schema 定义（MCP-1 上下文优先）。返回指定模型的结构、字段、关系。AI 助手在编写数据库查询代码时应调用此工具获取真实的 Schema 信息，避免凭记忆假设。',
      inputSchema: z.object({
        model: z
          .string()
          .optional()
          .describe(
            '可选的模型名称（不区分大小写）。不指定则返回所有模型概况。指定则返回该模型的完整字段定义。'
          ),
      }),
    },
      ...
      try {
        ...
        if (modelName) {
          ...
          const enumMatch = schema.match(
            new RegExp(`enum\\s+${modelName}\\s*\\{`, 'i')
          );
          if (enumMatch)
            return textResponse(
              `=== Enum: ${args.model} ===\n\n可用的枚举值见不传参调用结果。`
          ...
          return errorResponse(
            `未找到模型或枚举: ${args.model}。可用模型见不传参调用结果。`
          );
        }
        ...
        while ((m = modelRegex.exec(schema)) !== null) {
          ...
          if (braceIdx !== -1) {
            ...
            models.push({ name: m[1], fieldCount });
          }
        }
        const enums: string[] = [];
        ...
        parts.push(
          '',
          '提示: 指定 model 参数获取完整字段定义，例如: get_db_schema({ model: "User" })'
        );
        return textResponse(parts.join('\n'));
      } catch (error) {
        return errorResponse(
          `获取 Schema 失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "获取指定域的审计日志器完整代码或模式（MCP-1 上下文优先）。历史域（'knowledge-base', 'agent'）返回已有的混合式日志器代码；新域返回三层分离式模板（ADD-4 三层可插拔架构）:\n- Layer 1 开发审计（dev-logger）: console + file + DB metadata，可插拔\n- Layer 2 运行时审计（audit）: console + AuditLog 表，始终开启\n- Layer 3 调试日志: console only，LOG_LEVEL 控制\n新业务域必须使用三层分离模式。",
      inputSchema: z.object({
        domain: z
          .string()
          .describe(
            "审计日志器域: 'knowledge-base', 'agent'（历史混合式），或新域如 'personnel'（三层分离式）"
          ),
      }),
    },
    async (args: { domain: string }) => {
      try {
        ...
        const filePath = (() => {
          switch (domain) {
            case 'knowledge-base':
              return join(PROJECT_ROOT, 'src', 'lib', 'audit-logger.ts');
            case 'agent':
              return join(PROJECT_ROOT, 'src', 'lib', 'agent-audit-logger.ts');
            default:
              return '';
          }
        })();
        if (filePath) {
          ...
          if (!content)
            return errorResponse(
              `未找到 ${domain} 审计日志器文件: ${filePath}`
            );
          const meta: Record<
            string,
            { prefix: string; logDir: string; logFile: string }
          > = {
            'knowledge-base': {
              prefix: '[KB-AUDIT]',
              logDir: 'logs/knowledge-base/',
              logFile: 'kb-audit.log',
            },
            agent: {
              prefix: '[AGENT-AUDIT]',
              logDir: 'logs/agent/',
              logFile: 'agent-audit.log',
            },
          };
          const m = meta[domain];
          const parts = [
            ...
            `2. LOG_DIR: logs/domain/ 目录 (当前: ${m.logDir})`,
    ...
    async (args: { template: string }) => {
      try {
        ...
        if (template === 'list') {
          ...
          parts.push(
            '',
            '用法: get_add_template({ template: "plan-template" })'
          );
          return textResponse(parts.join('\n'));
        }
        ...
        return errorResponse(
          `获取 ADD 模板失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
    ...
    'get_spec_context',
    {
      ...
      inputSchema: z.object({
        task: z
          .string()
          .describe(
            `任务名（即 ${MAGIC_DIR}/specs/ 下的目录名），如 'co-agent-response-strategy'。传 'list' 获取所有任务列表。`
          ),
        file: z
          .string()
          .optional()
          .describe(
            "指定读取三元组中的某个文件: 'spec', 'tasks', 'checklist'。不传则返回全部三个文件。"
          ),
      }),
    },
      ...
      try {
        ...
        if (task === 'list') {
          ...
          for (const dir of dirs) {
            ...
            const hasChecklist = existsSync(
              join(specsDir, dir, 'checklist.md')
            );
            parts.push(
              `  ${hasTasks && hasChecklist ? '✅' : '⚠️ 不完整'} ${dir} — ${title}`
            );
          }
          parts.push(
            '',
            '用法: get_spec_context({ task: "co-agent-response-strategy" })'
          );
          return textResponse(parts.join('\n'));
        }
        ...
        const trinity: Record<string, string> = {
          spec: 'spec.md',
          tasks: 'tasks.md',
        ...
        if (file && trinity[file]) {
          const fp = join(taskDir, trinity[file]);
          const content = await readFileSafe(fp);
          if (content) {
            parts.push(`=== ${trinity[file]} ===`);
            parts.push(content);
          } else {
            parts.push(`⚠️ ${trinity[file]} 不存在`);
          }
        } else {
          for (const [_key, fn] of Object.entries(trinity)) {
            const fp = join(taskDir, fn);
            const content = await readFileSafe(fp);
            if (content) {
              parts.push(`=== ${fn} ===`);
              parts.push(content);
        ...
        return errorResponse(
          `获取 Spec 上下文失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
