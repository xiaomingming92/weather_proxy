  ...
  try {
    ...
    if (flag === 'allcity') {
      ...
      for (const city of cities) {
        xml += '  <City>\n';
        xml += `    <CityID>${city.cityId || ''}</CityID>\n`;
        xml += `    <CityName>${city.name}</CityName>\n`;
        xml += `    <StationID>${city.stationId || city.cityId || ''}</StationID>\n`;
        xml += `    <Lat>${city.latitude || ''}</Lat>\n`;
        xml += `    <Lon>${city.longitude || ''}</Lon>\n`;
        xml += '  </City>\n';
      }

      xml += '</CityList>';

      console.log(`Returning ${cities.length} cities in XML format`);
