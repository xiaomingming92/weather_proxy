...
// 已知的华风编码映射（从项目中获取）
const knownHuafengCodes: Record<string, string> = {
  '01011713': '北京',
  '01011714': '上海',
  '01011721': '重庆',
  '01012601': '天津',
...
function parseCSV(content: string): Array<{
  ...
  const headers = lines[1].split(','); // 第二行是表头
  const data: any[] = [];
...
// 和风编码转换为华风编码
function convertToHuafeng(heWeatherId: string): string | null {
  // 去掉 cn 前缀
...
function analyzeCodes() {
  ...
  const convertedCodes: Record<
    string,
    { heId: string; cityCn: string; provinceCn: string }
  > = {};
  ...
  // 5. 生成转换后的JSON
  console.log('\n=== 生成转换后的映射表 ===');
  const huafengToHeWeather: Record<
    string,
    { city: string; heWeatherId: string; lat: string; lon: string }
  > = {};
