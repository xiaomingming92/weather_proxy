...
function analyzeCodes() {
  ...
  for (const [huafengCode, expectedCity] of Object.entries(knownHuafengCodes)) {
    ...
    if (converted) {
      // 检查城市名是否匹配（或包含关系）
      const isMatch = converted.cityCn.includes(expectedCity) || 
  ...
  console.log(`匹配率: ${((matched / Object.keys(knownHuafengCodes).length) * 100).toFixed(2)}%`);
  
  // 3. 详细匹配结果
  console.log('\n=== 详细匹配结果 ===');
  for (const detail of matchDetails) {
    console.log(`${detail.status} ${detail.huafengCode} -> 期望: ${detail.expectedCity}, 实际: ${detail.matchedCity || 'N/A'}`);
  }
  ...
  for (const city of examples) {
    const huafengCode = convertToHuafeng(city.cityId);
    console.log(`${city.cityId} -> ${huafengCode || 'N/A'} (${city.cityCn}, ${city.provinceCn})`);
  }
  ...
  fs.writeFileSync(outputPath, JSON.stringify(huafengToHeWeather, null, 2), 'utf-8');
