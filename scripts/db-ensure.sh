...
# 0. 确保 .env.development 存在（仅写入 ADD/PostgreSQL 相关变量，Weather 业务的 MariaDB DATABASE_URL 需单独配置）
if [ ! -f "$PROJECT_DIR/.env.development" ]; then
  cat > "$PROJECT_DIR/.env.development" <<EOF
# === Weather 业务数据库 (MariaDB) ===
# WEATHER_DATABASE_URL 请按实际 MariaDB 连接信息配置
WEATHER_DATABASE_URL="mysql://admin:change-me-in-production@localhost:3306/weather-proxy"
...
# 快速检查：如果 PostgreSQL 已在运行且健康，跳过启动
